...
public class searchFlight {
    ...
    {
        Rectangle background = new Rectangle(width-200,height-450);
...
void showFoundFlights(Vector<Flight> flightsFound)
{
    ...
    {
        if(a>flightsFound.size()-1||a==-1)
            break;
        y += 0;
        ShowFlight(flightsFound.get(a), x, y, unite);
    }



    unite.getChildren().add(backgroundRecatngle());
    ...
    next.setOnAction(new EventHandler<ActionEvent>() {
        public void handle(ActionEvent t) {
            if(pageNr!=flightsFound.size()-1) {
                if (pageNr + maxPerPage < flightsFound.size())
                    pageNr += maxPerPage;
                else
                    pageNr += flightsFound.size()-(flightsFound.size()-maxPerPage);
                if(pageNr>=flightsFound.size())
                {
                    pageNr -= flightsFound.size()-(flightsFound.size()-maxPerPage);
                    return;
                }
...
Vector<Flight> findFlights(Date flightDate,String destination)
{
    Vector<Flight> flights =new Vector<>() ;
    for(Flight a : ReadDB.getFlightList())
    {
        if(a.getM_id_destinationTo().getCity().equals(destination))
            if(a.getM_date().compareTo(flightDate)==0)
                flights.add(a);
    }
    ...
    private void form()
    {
        ...
        for(String city: ReadDB.getAllCities())
            destination.getItems().add(city);
        destination.setMaxHeight(30);
        destination.setMaxWidth(250);
        destination.setTranslateX(180);
        destination.setTranslateY(230);

        Label destinationLabel = new Label("Destinatia: ");
        destinationLabel.setStyle("-fx-font-size: 20px;-fx-font-weight: bold;-fx-text-fill: white;");
        destinationLabel.setTranslateX(180);
        destinationLabel.setTranslateY(190);

        DatePicker dataPicker = new DatePicker();
        dataPicker.setMaxHeight(30);
        dataPicker.setMaxWidth(350);
        dataPicker.setTranslateX(350);
        dataPicker.setTranslateY(230);

        Label dataLabel = new Label("Data: ");
        dataLabel.setStyle("-fx-font-size: 20px;-fx-font-weight: bold;-fx-text-fill: white;");
        dataLabel.setTranslateX(360);
        dataLabel.setTranslateY(190);


        Button submit = new Button("       Cauta");
        submit.setId("searchButton");
        submit.setTranslateX(dataPicker.getTranslateX()+250);
        submit.setTranslateY(dataPicker.getTranslateY()-10);
        submit.setPrefSize(120,20);

        ImageView searchIcon =new ImageView("C:\\Users\\40763\\IdeaProjects\\AirportMip\\src\\main\\resources\\com\\example\\airportmip\\searchIcon.png");
        searchIcon.setFitHeight(20);
        searchIcon.setFitWidth(20);
        submit .setGraphic(searchIcon);

        submit.setOnAction(t -> {
            Alert error = new Alert(Alert.AlertType.WARNING);

            if(dataPicker.getValue()==null) {
                error.setContentText("Selectati data zborului!");
                error.show();
                return;
            }
            if(destination.getValue()==null) {
                error.setContentText("Selectati destinatia zborului!");
                error.show();
                return;
            }

            ZoneId defaultZoneId = ZoneId.systemDefault();
            Date flightDate = Date.from(dataPicker.getValue().atStartOfDay(defaultZoneId).toInstant());
            String destinationChosen = destination.getValue().toString();
            Vector<Flight> flightsFound = findFlights(flightDate,destinationChosen);
            if(flightsFound.size()>0)
            {
                root.getChildren().clear();
                showFoundFlights(flightsFound);
                Menu();
                Logo();
            }

            else {
                error.setContentText("Nu am gasit zboruri in aceasta zi!");
                error.show();
                return;
            }
        ...
        root.getChildren().add(dataPicker);
        root.getChildren().add(destinationLabel);
